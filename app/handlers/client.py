...
async def create_ticket_handler(message: Message, session: AsyncSession = None):
    ...
    await message.answer(f"Тикет #{new_ticket.id} создан и установлен как активный. "
...
async def my_tickets_handler(message: Message, session: AsyncSession = None):
    ...
    await message.answer("Ваши тикеты:", reply_markup=await client_kb.get_user_tickets_kb(tickets))
...
async def active_ticket_menu_handler(message: Message, session: AsyncSession = None):
    ...
    if not active_ticket_id:
        await message.answer("У вас нет активного тикета. Создайте новый или выберите из списка существующих.")
        return
        
    await message.answer(f"Текущий активный тикет: #{active_ticket_id}", reply_markup=client_kb.get_active_ticket_menu(active_ticket_id))
...
async def reopen_ticket_callback(query: CallbackQuery, callback_data: client_kb.TicketCallback, session: AsyncSession = None):
    ...
    await crud.set_active_ticket(session, query.from_user.id, ticket_id)
    await query.message.edit_text(f"Тикет #{ticket_id} был переоткрыт и установлен как активный.")
...
async def subscription_status_handler(message: Message, session: AsyncSession = None):
    ...
    if not subscription:
        await message.answer("Информация о вашей подписке не найдена. Обратитесь в поддержку.")
        return
        
    end_date = subscription.end_date
    days_left = (end_date.date() - datetime.utcnow().date()).days
    
    response = (f"Ваша подписка активна до: {end_date.strftime('%d.%m.%Y')}\n"
...
async def handle_message_in_ticket(message: Message, bot: Bot, session: AsyncSession = None):
    ...
    if ticket.status == TicketStatus.CLOSED:
        await message.answer("Этот тикет закрыт. Вы не можете отправлять в него сообщения. "
                             "Переоткройте его или создайте новый.",
                             reply_markup=await client_kb.get_user_tickets_kb(await crud.get_user_tickets(session, message.from_user.id)))
        return
    ...
    await crud.add_message_to_ticket(session, active_ticket_id, message.from_user.id, content_type, text, file_id)
    ...
    for admin_id in settings.admin_ids:
        with contextlib.suppress(Exception):
            notification_text = (f"Новое сообщение в тикете #{active_ticket_id} "
                                 f"от @{message.from_user.username} ({message.from_user.id})")
            await bot.send_message(admin_id, notification_text, reply_markup=admin_kb.get_ticket_actions_kb(active_ticket_id, message.from_user.id))
